(ns gasfield.operation
  "OperationActor -- one natural-gas-extraction operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (GasFieldAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Gas Well Safety Governor
  ...
  One graph run = one gas operation (intake -> advise -> govern ->
  decide -> commit | hold | approval). No unbounded inner loop -- each
  operation is auditable and checkpointed. A gas well's life is advanced
  by MANY ops (assess / extract / settle), each its own independent run.

  Human-in-the-loop = real approval workflow:
  `interrupt-before #{:request-approval}` pauses the actor and hands the
  decision to a human operator (well operator / production
  superintendent). The approver resumes with `{:approval {:status
  :approved}}` (or :rejected). `:well/extract`/`:production/settle`
  ALWAYS reach this node when the governor is clean -- see
  `gasfield.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [gasfield.gasfieldadvisor :as gasfieldadvisor]
            [gasfield.governor :as governor]
...
(defn build
  ...
  `gasfield.store/Store`).
  opts:
    :advisor      -- a `gasfield.gasfieldadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (gasfieldadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; GasFieldAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (gasfieldadvisor/-advise advisor store request)]
            {:proposal p :audit [(gasfieldadvisor/trace request p)]})))

      ;; Gas Well Safety Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
